 ...
 * dependencies: none (vanilla JS — no jQuery). reads window.videoGuideConfig if the
 *               host page provides it; every key has a shipped fallback.
 * key functions: initVideoGuide(), applyConfig(), setPercentage(), getRingProgress(),
 *                showBubble(), scheduleBubble(), scheduleBubbleHide(), runReveal(),
 *                revealInstant(), finishReveal(), publishRevealTiming()
...
// overridable per page via window.videoGuideConfig.revealDelayMs — see applyConfig().
...
function initVideoGuide() {
	...
	/**
	 * applyConfig()
	 * purpose: replace the widget's shipped demo media and greeting with host values
	 * input: none (reads window.videoGuideConfig)
	 * output: none — rewrites the <source> srcs, the splash img src/alt and the chat
	 *         bubble text in place
	 * dependencies: the element references above. must run before any listener is
	 *               attached, so nothing can fire against the demo media.
	 * why: widget.html ships hardcoded fallbacks so the dev pages work standalone. on
	 *      a wordpress job offer page every one of them is wrong — injected markup
	 *      resolves relative urls against the *page* url, so `assets/…` becomes
	 *      /jobs/<slug>/assets/… and 404s, and the video and greeting are chosen per
	 *      offer page by an editor.
	 * note: reads with getAttribute(), never the .src property — the property returns
	 *       the already-resolved absolute url, which is the very thing being corrected.
	 *       bubbleText is written with textContent, never innerHTML: it is editor input
	 *       and this is the only place untrusted text enters the widget.
	 */
	function applyConfig() {
		const cfg     = window.videoGuideConfig || {};
		const baseUrl = cfg.baseUrl || '';

		/**
		 * resolve()
		 * purpose: decide the final url for one asset attribute
		 * input: el (Element), attr (string), override (string|undefined) — the config value
		 * output: (string|null) the url to write, or null for "leave it alone"
		 * an explicit config url wins outright. otherwise the markup's own relative path
		 * is kept and prefixed with baseUrl. absolute and root-relative paths are left
		 * untouched — only a document-relative path is ambiguous enough to need a base.
		 */
		function resolve(el, attr, override) {
			if (override) return override;
			const raw = el.getAttribute(attr);
			if (!baseUrl || !raw) return null;
			if (/^(?:[a-z]+:)?\/\//i.test(raw) || raw.charAt(0) === '/') return null;
			return baseUrl + raw;
		}

		// -> video sources
		// reassigning a <source>'s src has no effect on an already-parsed <video> until
		// load() runs — but calling load() unconditionally would discard the dev pages'
		// in-flight request and refetch the same file for nothing. so: only if changed.
		let sourcesChanged = false;

		[['source[type="video/webm"]', cfg.videoWebm],
		 ['source[type="video/mp4"]',  cfg.videoMp4]].forEach(function(pair) {
			const sourceEl = video.querySelector(pair[0]);
			if (!sourceEl) return;
			const next = resolve(sourceEl, 'src', pair[1]);
			if (next && next !== sourceEl.getAttribute('src')) {
				sourceEl.setAttribute('src', next);
				sourcesChanged = true;
			}
		});

		if (sourcesChanged) video.load();

		// -> splash image
		const splashImg = splashEl.querySelector('.video-guide__splash-avatar');
		if (splashImg) {
			const nextSrc = resolve(splashImg, 'src', cfg.splashImage);
			if (nextSrc) splashImg.setAttribute('src', nextSrc);
			if (cfg.splashAlt) splashImg.setAttribute('alt', cfg.splashAlt);
		}

		// -> greeting
		const bubbleTextEl = chatBubble.querySelector('.video-guide__chat-bubble-text');
		if (bubbleTextEl && cfg.bubbleText) bubbleTextEl.textContent = cfg.bubbleText;
	}

	applyConfig();

	// pre-roll delay, host-overridable. the reveal hangs off window.load, which on an
	// image-heavy job offer page fires late enough that the default 1500ms on top of it
	// is the first thing worth lowering. isFinite() rather than a truthiness check so a
	// deliberate 0 is honoured instead of falling back to 1500.
	const cfgDelay = (window.videoGuideConfig || {}).revealDelayMs;
	const revealDelayMs = Number.isFinite(cfgDelay) ? cfgDelay : REVEAL_DELAY_MS;
	...
	// -> reveal trigger
	// window.load + pre-roll delay (revealDelayMs, resolved at init).
	//
	// readyState is checked first because this whole function runs off the
	// videoGuideWidgetLoaded event, i.e. after widget-loader.js's fetch resolves. on a
	// slow host page that fetch can easily land *after* window.load has already fired,
	// and a listener registered for an event that is already in the past never runs —
	// the widget would sit parked below the viewport forever. this is the more likely
	// order on a real job offer page, not the exotic one.
	if (document.readyState === 'complete') {
		setTimeout(runReveal, revealDelayMs);
	} else {
