...
Async flow: `widget-loader.js` (IIFE) → `fetch(cfg.baseUrl + 'widget.html')` → `insertAdjacentHTML('beforeend', ...)` → `dispatchEvent('videoGuideWidgetLoaded')` → `video-guide.js` runs `initVideoGuide()` → `applyConfig()`.

For deep dives, read [docs/architecture.md](docs/architecture.md).

## Host page configuration

`window.videoGuideConfig` is an optional global the host page may set **before** `widget-loader.js`
loads. Keys: `baseUrl`, `videoWebm`, `videoMp4`, `splashImage`, `splashAlt`, `bubbleText`,
`revealDelayMs`. Every one falls back to what `widget.html` ships, so the dev pages need no config.

`baseUrl` is required on WordPress: injected markup resolves relative URLs against the *page* URL, so
`assets/…` would become `/jobs/<slug>/assets/…`. `applyConfig()` in `js/video-guide.js` repairs that
by prefixing `baseUrl`, and reads attributes with `getAttribute()` — never the `.src` property, which
returns the already-resolved URL. `bubbleText` is WordPress editor input and goes in via
`textContent`, never `innerHTML`.

Full contract, per-offer editor field mapping and the staging test protocol: [docs/handover.md](docs/handover.md).

## Distribution

`dist/video-guide/` is the deployable tree for `karriere.hypoport.de` (staging first). Built by
`scripts/build-dist.sh`, which copies `widget.html`, `js/`, `scss/base.scss`, `assets/`, `docs/` and
`HANDOVER.md`, and writes `VERSION`.

**The CSS is not built by that script.** `.vscode/settings.json` configures Live Sass Compile to emit
two outputs on every save: `compressed` → `css/`, `expanded` → `dist/video-guide/css/`. So the order
is always **save `scss/base.scss` in VS Code, then run the script** — it aborts if the dist CSS is
missing or stale. Distribution CSS is never minified and the SCSS source ships with it, because Sass
strips `//` comments from every output format and on-host tooling needs that commentary.

`sample/` is dev-only and is not distributed; the two runtime images live in `assets/`.
...
- **Keep base.scss host-safe.** Every selector goes under `.video-guide`, every custom property is `--vg-`-prefixed, every `@keyframes` is `vg-`-prefixed. Styling that belongs to a dev page goes in that page's own `<style>` block — see index.html.
...
- Use `innerHTML` with anything that isn't a trusted hardcoded template (XSS) — `bubbleText` is editor input and uses `textContent`
- Re-introduce jQuery (current widget is pure vanilla JS)
- Add a top-level selector to base.scss that isn't `.video-guide`-scoped, `:root`, or a `vg-`-prefixed `@keyframes` — it would leak onto host pages
- Edit anything under `dist/` by hand; it is generated by `scripts/build-dist.sh`
...
- [docs/handover.md](docs/handover.md) — distribution handover for the karriere.hypoport.de job portal plugin team: config contract, load order, media requirements, staging test protocol. Copied into the export as `HANDOVER.md`; edit it here, never there.
